...
export class PlayerComparisonService {
    ...
    async compareHeadToHead(
        ...
        const sharedMatches = matches.filter(
            m =>
                m.players[player1Name] &&
                m.players[player2Name],
        )
    ...
    private calculateAdvantages(
        ...
        const playerkD = playerStats.deaths > 0 ? playerStats.frags / playerStats.deaths : playerStats.frags
        const opponentkD = opponentStats.deaths > 0 ? opponentStats.frags / opponentStats.deaths : opponentStats.frags
